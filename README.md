...
> A little system tray utility that lets you pick a display mode for external monitors instead of defaulting to clone
...
1. Run `monitor-monitor.exe` or `python -m monitor_monitor` depending on how you got your hands on it
...
```
monitor-monitor.exe [mode] [--minimized]
```

Modes:
| Argument | Mode |
|----------|------|
| `internal` or `1` | Internal Only |
| `clone` or `2` | Clone |
| `extend` or `3` | Extend (default) |
| `external` or `4` | External Only |
...
Examples:
```
monitor-monitor.exe clone --minimized
monitor-monitor.exe 3
```
...
2. Create a new shortcut and point it to wherever you put `monitor-monitor.exe` along with any command line arguments you want to use
...
- **Not instant** - the polling means there's a 1-3 second delay when connecting a new screen
...
Monitor Monitor polls for connected displays every 3 seconds using a PowerShell query against WMI (`WmiMonitorBasicDisplayParams`). When the count changes, it invokes `DisplaySwitch.exe` with your preferred mode.
...
### Why PyInstaller? I'm sure you could make it smaller than 18MB and more elegant.
